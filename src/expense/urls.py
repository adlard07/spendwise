...
from src.expense.services import (
    create_expense,
    delete_expense,
    get_expense,
    get_expenses,
...
async def create_new_expense(expense: CreateExpense):
    """Create new expense."""
    try:
...
async def fetch_expense(expense_id: str):
    """Get expense by ID."""
...
async def fetch_expenses_by_category(
    user_id: str, category_id: str, limit: Optional[int] = Query(None, ge=1, le=100)
):
...
async def fetch_recurring_expenses(user_id: str):
...
async def fetch_total_expenses(user_id: str):
    """Calculate total expenses for a user."""
    try:
        total = get_total_expenses(user_id=user_id)
        return {"success": True, "data": {"user_id": user_id, "total_expenses": total}}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_expenses(expense_id: str, update_data: UpdateExpense):
    """Update expense."""
